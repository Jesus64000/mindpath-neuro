...
const ConsultationRoom = () => {
  ...
  useEffect(() => {
    api
      .get(`/appointments/doctor/${appointmentId}/detail`)
  ...
  const endConsultation = () => {
    ...
    // Guardamos también lo último interceptado
    if (fullText) {
      localStorage.setItem(`transcription_${appointmentId}`, fullText);
    }
      ...
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6">
        <div className="text-center max-w-md">
          ...
          <p className="text-slate-400 text-sm">
            La transcripción requiere Chrome o Edge.
          </p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-slate-900 text-white flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col p-6 overflow-hidden">
          ...
          <div className="flex-1 bg-white/5 rounded-2xl border border-white/10 p-6 overflow-y-auto text-sm leading-relaxed">
            {!finalText && !interimText ? (
              <p className="text-slate-500 italic select-none">
                {isListening
                  ? "Escuchando… habla claramente frente al micrófono."
                  : "Pulsa el botón de micrófono para iniciar la transcripción."}
              </p>
            ) : (
              <p>
          ...
          {error && (
            <div className="mt-3 text-red-400 text-xs bg-red-500/10 border border-red-500/20 rounded-xl px-4 py-2">
              {error}
            </div>
          )}
        </div>
        ...
        <div className="w-72 border-l border-white/10 p-5 flex flex-col gap-5">
          <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
            ...
            <p className="text-xs text-slate-500">
              Sesión presencial — transcripción automática activa
            </p>
          </div>
